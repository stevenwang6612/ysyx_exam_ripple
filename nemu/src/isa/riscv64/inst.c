...
static void decode_operand(Decode *s, word_t *dest, word_t *src1, word_t *src2, int type) {
  ...
  switch (type) {
    ...
    case TYPE_C: src1I(rs1);     src2I(immI(i)); break;
    default: break;
  }
}
...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = src1 < BITS(src2, 11, 0));
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , I, R(dest) = src1 >> BITS(src2, 5, 0));
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = (word_t)((sword_t)src1 >> BITS(src2, 5, 0)));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", srlw   , R, R(dest) = (word_t)SEXT((sword_t)SEXT(BITS(src1, 31, 0), 32) >> BITS(src2, 4, 0), 32));
  INSTPAT("0000001 ????? ????? 000 ????? 01110 11", mulw   , R, R(dest) = SEXT(src1 * src2, 32));
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw   , R, R(dest) = SEXT(src1 / src2, 32));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11", sraiw  , I, R(dest) = (word_t)SEXT((sword_t)SEXT(BITS(src1, 31, 0), 32) >> BITS(src2, 4, 0), 32));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(dest) = (word_t)((sword_t)src1 >> BITS(src2, 4 ,0)));
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(dest) = src1 | src2);
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , R, R(dest) = src1 & src2);
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(dest) = src1 * src2);
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(dest) = (word_t)(((sdword_t)src1 * (sdword_t)src2)>>64));
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(dest) = (word_t)(((sdword_t)src1 * (dword_t)src2)>>64));
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(dest) = (word_t)(((dword_t)src1 * (dword_t)src2)>>64));
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(dest) = (word_t)((sword_t)src1 / (sword_t)src2));
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(dest) = src1 / src2);
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(dest) = (word_t)((sword_t)src1 % (sword_t)src2));
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, R(dest) = src1 % src2);
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = s->snpc, s->dnpc = src1 + s->pc);
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11100 11", csrrwi , C, R(dest) = CSR(src2), CSR(src2) = src1);
  INSTPAT("??????? ????? ????? 110 ????? 11100 11", csrrsi , C, R(dest) = CSR(src2), CSR(src2) |= src1);
  INSTPAT("??????? ????? ????? 111 ????? 11100 11", csrrci , C, R(dest) = CSR(src2), CSR(src2) &= ~src1);
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, CSR(CSR_MEPC)=s->pc, CSR(CSR_MCAUSE)= 0xb, s->dnpc=CSR(CSR_MTVEC)); 
