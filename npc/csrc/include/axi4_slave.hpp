        ...
        void read_beat(axi4_ref <A_WIDTH,D_WIDTH,ID_WIDTH> &pin) {
            ...
            if (update) {
                if (r_early_err) {
                    ...
                    pin.rresp = mmio_read(r_start_addr, &pin.rdata) ? RESP_OKEY : RESP_DECERR;
                }
    ...
    private:
        ...
        void write_beat(axi4_ref <A_WIDTH,D_WIDTH,ID_WIDTH> &pin) {
            if (pin.wvalid && pin.wready) {
                ...
                if (w_start_addr > DEVICE_BASE) {
                    w_resp = mmio_write(w_start_addr, pin.wdata, w_each_len) ? RESP_OKEY : RESP_DECERR;
                }
